...
export default function PetsLayout() {
  ...
  const species = speciesTypes.map((btn) => (
    <button key={btn} onClick={() => handleFilterChange('type', btn)} className='filter-btn'>
      {btn}
    </button>
  ));
  const breed = breedTypes.map((btn) => (
    <button key={btn} onClick={() => handleFilterChange('breed', btn)} className='filter-btn'>
      {btn}
    </button>
  ));
  const ages = ageTypes.map((btn) => (
    <button key={btn} onClick={() => handleFilterChange('age', btn)} className='filter-btn'>
      {btn}
    </button>
  ));
  const genders = genderTypes.map((btn) => (
    <button key={btn} onClick={() => handleFilterChange('gender', btn)} className='filter-btn'>
      {btn}
    </button>
  ));
